...
{{
	...
	"TextureColor"	"{color}"
	"TextureAmbientOcclusion"	"{d}/fv_white.png"
	"TextureNormalRoughness"	"{d}/fv_flat_normal.png"
	"TextureNprOutlineMask"	"{d}/fv_white.png"
	"TextureNprTransmissiveColor"	"{d}/fv_black.png"
	"TextureSelfIllumMask"	"{d}/fv_black.png"
	"TextureTintMask"	"{d}/fv_white.png"
	"TextureTintMaskRimLightMask"	"{d}/fv_rimlight_mask.png"
}}
"""

# 1x1 helper maps, same values the stock hero materials use
HELPER_PNGS = {
    "fv_white.png": (1, 1, 1, 1),
    "fv_black.png": (0, 0, 0, 1),
    "fv_flat_normal.png": (0.5, 0.5, 1.0, 0.75),   # flat normal, roughness in alpha
    "fv_rimlight_mask.png": (1, 1, 0, 1),
}
...
def main():
    ...
    for fn, rgba in HELPER_PNGS.items():
        write_png(os.path.join(out_mat_dir, fn), 1, 1, rgba)
    used, done = set(), {}
    for o in keep:
        if o == gun:
            continue
        ...
        for i, m in enumerate(o.data.materials):
            ...
            with open(os.path.join(addon_root, vmat_rel), "w", encoding="utf-8") as f:
                f.write(VMAT_TEMPLATE.format(color="%s/%s" % (MAT_DIR, color_png), d=MAT_DIR, backfaces=0))
